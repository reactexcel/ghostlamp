...
export default class AgreeScreen extends Component {
  ...
  render() {
    ...
    const { next, back, onChange } = this.props;
    return (
      <View style={styles.container}>
        ...
        <View style={{marginTop:25}} >
            ...
            { terms?
                    ...
                    <Icon name="checkmark-circle" style={{height:20,width:20,fontSize:18,color:'green'}}  onPress={()=>{ onChange('terms') }} />
                    </View>
                :
                    <TouchableOpacity onPress={()=>{ onChange('terms')}} >
            ...
            { policy?
                    <View style={{height:15,width:15}} >
                    <Icon name="checkmark-circle" style={{height:20,width:20,fontSize:18,color:'green'}}  onPress={()=>{ onChange('policy') }} />
                    </View>
                :
                    <TouchableOpacity onPress={()=>{  onChange('policy') }} >
                        <View style={{borderWidth:1,borderRadius:20,height:15,width:15,borderColor:'gray'  }} />
                    </TouchableOpacity>
                }
