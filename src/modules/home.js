  ...
  render() {
    ...
    return (
      <View style={styles.container}>
          ...
          <Button first style={Platform.OS == 'android' ? {width:100,borderColor:'gray',height:40,borderRightWidth:0,justifyContent:'center',borderTopLeftRadius:10,borderBottomLeftRadius:10}:{width:100,height:40,borderColor:'gray',borderRightWidth:0,justifyContent:'center'}} active={selection == 0?true:false} onPress={()=>{ this.setState({selection:0}) }}  >
            <Text style={[selection == 0 ? {color:TextColor,fontSize:16}:{fontSize:16}]} >Balance</Text>
          </Button>
          <Button style={{width:100,height:40,borderRightWidth:0,borderColor:'gray',justifyContent:'center'}} active={selection == 1?true:false} onPress={()=>{ this.setState({selection:1}) }} >
            <Text style={[selection == 1 ? {color:TextColor,fontSize:16}  :{fontSize:16}]} >Actions</Text>
          </Button>
          <Button last style={Platform.OS == 'android'?{width:150,borderColor:'gray',height:40,justifyContent:'center',borderBottomRightRadius:10,borderTopRightRadius:10}:{width:150,borderColor:'gray',height:40,justifyContent:'center'}} active={selection == 2?true:false} onPress={()=>{ this.setState({selection:2}) }} >
